...
//     // Utility: Load STL file into an FCL BVHModel
//     std::shared_ptr<fcl::BVHModel<fcl::OBBRSSd>> loadSTLAsFCLModel(const std::string& stl_path)
//     {
//         Assimp::Importer importer;
//         const aiScene* scene = importer.ReadFile(stl_path, aiProcess_Triangulate | aiProcess_JoinIdenticalVertices);
//         if (!scene || !scene->HasMeshes())
//             throw std::runtime_error("Failed to load STL: " + stl_path);

//         std::shared_ptr<fcl::BVHModel<fcl::OBBRSSd>> model(new fcl::BVHModel<fcl::OBBRSSd>());
//         model->beginModel();
...
//                 fcl::Vector3d v1(mesh->mVertices[face.mIndices[0]].x,
//                                 mesh->mVertices[face.mIndices[0]].y,
//                                 mesh->mVertices[face.mIndices[0]].z);
//                 fcl::Vector3d v2(mesh->mVertices[face.mIndices[1]].x,
//                                 mesh->mVertices[face.mIndices[1]].y,
//                                 mesh->mVertices[face.mIndices[1]].z);
//                 fcl::Vector3d v3(mesh->mVertices[face.mIndices[2]].x,
...
//     static shape_msgs::msg::Mesh loadSTLAsShapeMsgMesh(const std::string& stl_path)
//     {
//         Assimp::Importer importer;
//         const aiScene* scene = importer.ReadFile(stl_path, aiProcess_Triangulate | aiProcess_JoinIdenticalVertices);
//         if (!scene || !scene->HasMeshes())
...
//         // Asumimos que el STL trae una única malla grande (habitual para una habitación)
//         for (unsigned int m = 0; m < scene->mNumMeshes; ++m)
//         {
//             const aiMesh* msh = scene->mMeshes[m];

//             // acumula offset para índices si hubiera varias submeshes
//             uint32_t base_index = static_cast<uint32_t>(mesh_msg.vertices.size());

//             // vértices
//             mesh_msg.vertices.reserve(mesh_msg.vertices.size() + msh->mNumVertices);
//             for (unsigned int v = 0; v < msh->mNumVertices; ++v)
//             {
//                 geometry_msgs::msg::Point p;
//                 p.x = msh->mVertices[v].x;
//                 p.y = msh->mVertices[v].y;
//                 p.z = msh->mVertices[v].z;
//                 mesh_msg.vertices.push_back(p);
//             }

//             // triángulos
//             mesh_msg.triangles.reserve(mesh_msg.triangles.size() + msh->mNumFaces);
//             for (unsigned int f = 0; f < msh->mNumFaces; ++f)
//             {
//                 const aiFace& face = msh->mFaces[f];
//                 if (face.mNumIndices != 3) continue;
//                 shape_msgs::msg::MeshTriangle tri;
//                 tri.vertex_indices[0] = base_index + face.mIndices[0];
//                 tri.vertex_indices[1] = base_index + face.mIndices[1];
//                 tri.vertex_indices[2] = base_index + face.mIndices[2];
...
//     try
//     {
//         std::string stl_path = "/home/yeray/ws_collision/src/example_building.stl";

//         // 1) Cargar STL → shape_msgs::Mesh
//         shape_msgs::msg::Mesh room_mesh = loadSTLAsShapeMsgMesh(stl_path);

//         // 2) Pose fija en base_link_ (ajústalo si lo necesitas)
//         Eigen::Affine3d room_pose = Eigen::Affine3d::Identity();
//         // p.ej.: room_pose.translation() = Eigen::Vector3d(0.0, 0.0, 0.0);
//         room_pose.translation() = Eigen::Vector3d(2.0, 0.0, 0.0);

//         // 3) Crear FCLObject con tipo MESH y añadirlo al mundo
//         robot_collision_checking::FCLObjectPtr room_fcl_obj =
//             std::make_shared<robot_collision_checking::FCLObject>(
//                 room_mesh, robot_collision_checking::MESH, room_pose);

//         collision_world_->addCollisionObject(room_fcl_obj, 999);
//         RCLCPP_INFO(this->get_logger(), "Room STL loaded successfully into collision world.");
//     }
//     catch (const std::exception& e)
//     {
//         RCLCPP_ERROR(this->get_logger(), "Error loading room STL: %s", e.what());
//     }

//     // Create parameter client to grab the robot_description from robot_state_publisher
//     std::string rsp_node_name = this->declare_parameter<std::string>(
//         "robot_state_publisher_node", "/robot_state_publisher");

//     auto params_client = std::make_shared<rclcpp::SyncParametersClient>(this, rsp_node_name);

//     // Esperar hasta 10 segundos máximo al servicio
//     RCLCPP_INFO(this->get_logger(), "Waiting for service %s...", rsp_node_name.c_str());
...
//             break;
//         }
//         RCLCPP_WARN(this->get_logger(), "Still waiting for %s...", rsp_node_name.c_str());
//     }
//     if (!service_ready) {
//         RCLCPP_ERROR(this->get_logger(), "Timeout waiting for %s service. Continuing without robot description.", rsp_node_name.c_str());
//         return;
//     }

//     // Intentar obtener el parámetro robot_description
...
//             {
//                 robot_desc_string = parameters[0].as_string();
//                 got_robot = true;
//                 break;
//             }
//         } catch (const std::exception &e) {
//             RCLCPP_WARN(this->get_logger(), "Exception while fetching robot_description: %s", e.what());
//         }

//         RCLCPP_WARN(this->get_logger(), "Retrying to fetch robot_description...");
//         std::this_thread::sleep_for(std::chrono::milliseconds(500));
//     }

//     if (!got_robot) {
//         RCLCPP_ERROR(this->get_logger(), "Failed to retrieve 'robot_description' from %s. Proceeding without URDF.", rsp_node_name.c_str());
//         return;
//     }

//     RCLCPP_INFO(this->get_logger(), "Robot description successfully retrieved (%zu chars).", robot_desc_string.size());
